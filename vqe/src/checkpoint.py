...
class ExportCheckpoint(Callback):
    ...
    def save_model(self, trainer: "pl.Trainer", filepath):
        raise Exception('save model is not overridden')
    ...
    def _save_last_checkpoint(self, trainer: "pl.Trainer", monitor_candidates: Dict[str, _METRIC]) -> None:
        ...
        self.save_model(trainer, filepath)
...
class ONNXExportCallback(ExportCheckpoint):
    def __init__(self, *args, **kwargs):
        super(ONNXExportCallback, self).__init__(*args, **kwargs)
        self.FILE_EXTENSION = '.onnx'

    def save_model(self, trainer, filepath):
        trainer.lightning_module.save_onnx(filepath)


def get_resume_chkpt(resume_from):
    training_resume_chkpts = glob.glob('checkpoints/%s*' % (resume_from))
    if not training_resume_chkpts:
        # A new 'retry_xxx' is created on every preemption on AML
        # To fetch latest resume checkpoint, either go to 'retry_*'
        # Or the directory before preemption

        training_resume_chkpts = glob.glob('../../train/checkpoints/%s*' % (resume_from)) + \
                                 glob.glob('../../retry_*/train/checkpoints/%s*' % (resume_from))

    if not training_resume_chkpts:
        resume_chkpt = None
    else:
        resume_chkpt = max(training_resume_chkpts,  key=os.path.getctime)
    return resume_chkpt